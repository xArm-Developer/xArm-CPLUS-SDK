...
void ReportDataDevelop::print_data(void) {
  XARM_LOG_PURE("total  = %d\n", total_num_);
  XARM_LOG_PURE("runing = %d\n", runing_);
  XARM_LOG_PURE("mode   = %d\n", mode_);
  XARM_LOG_PURE("cmdnum = %d\n", cmdnum_);
...
int ReportDataNorm::flush_data(unsigned char *rx_data) {
  ...
  if (total_num_ != 133 && total_num_ != 145) { 
    XARM_LOG_ERROR("total_num=%d, sizeof_data=%d\n", total_num_, sizeof_data);
    return -2; 
  }
...
void ReportDataNorm::print_data(void) {
  ...
  XARM_LOG_PURE("mode    = %d\n", mode_);
  XARM_LOG_PURE("mt_brake= %x\n", mt_brake_);
  XARM_LOG_PURE("mt_able = %x\n", mt_able_);
  XARM_LOG_PURE("err&war = %d %d\n", err_, war_);
  print_nvect("tcp_off = ", tcp_offset_, 6);
  print_nvect("tap_load= ", tcp_load_, 4);
  XARM_LOG_PURE("coll_sen= %d\n", collis_sens_);
  XARM_LOG_PURE("teac_sen= %d\n", teach_sens_);
...
void ReportDataRich::print_data(void) {
  ...
  XARM_LOG_PURE("xarm_type = %d(axis%d)\n", arm_type_, axis_num_);
  XARM_LOG_PURE("xarm_msid = 0x%X 0x%X\n", master_id_, slave_id_);
  XARM_LOG_PURE("motor_tfid = 0x%X 0x%X\n", motor_tid_, motor_fid_);

  print_nvect("trs_msg = ", trs_msg_, 5);
  print_nvect("p2p_msg = ", p2p_msg_, 5);
  print_nvect("ros_msg = ", rot_msg_, 2);

  XARM_LOG_PURE("ID   Status  ErrorCode\n");
  for (int i = 0; i < 8; i++) {
    XARM_LOG_PURE("%d      %d        0x%X\n", i + 1, sv3msg_[i * 2],
           sv3msg_[i * 2 + 1]);
  }
}
...
{
  XARM_LOG_PURE("total = %d\n", total_num);
  XARM_LOG_PURE("state = %d, mode = %d, cmdnum = %d\n", state, mode, cmdnum);
...
{
  __print_common_data();
  XARM_LOG_PURE("mt_brake = 0x%X, mt_able = 0x%X\n", mt_brake, mt_able);
  XARM_LOG_PURE("error = %d, warn = %d\n", err, war);
  XARM_LOG_PURE("coll_sen = %d, teach_sen = %d\n", collis_sens, teach_sens);
...
{
  _print_normal_data();
  XARM_LOG_PURE("xarm_axis = %d, xarm_type = %d\n", axis_num, arm_type);
  XARM_LOG_PURE("master_id = 0x%X, slave_id = 0x%X\n", master_id, slave_id);
  XARM_LOG_PURE("motor_tid = 0x%X, motor_fid = 0x%X\n", motor_tid, motor_fid);
  ...
  XARM_LOG_PURE("ID   State  ErrorCode\n");
  for (int i = 0; i < 8; i++) {
  ...
  if (total_num >= 284) {
    XARM_LOG_PURE("realtime_tcp_speed: %f\n", rt_tcp_spd);
    print_nvect("realtime_joint_speed: ", rt_joint_spds, 7);
  }
  if (total_num >= 288) {
    XARM_LOG_PURE("counter_val: %d\n", count);
  }
  if (total_num >= 312) {
    print_nvect("world_offset: ", world_offset, 6);
  }
  if (total_num >= 314) {
    XARM_LOG_PURE("gpio_reset_conf: [ %d, %d ]\n", gpio_reset_conf[0], gpio_reset_conf[1]);
  }
  if (total_num >= 417) {
    XARM_LOG_PURE("simulation_mode: %d\n", simulation_mode);
    XARM_LOG_PURE("collision_detection: %d\n", collision_detection);
    XARM_LOG_PURE("collision_tool_type: %d\n", collision_tool_type);
    ...
    XARM_LOG_PURE("CGPIO_INPUT: \n");
      XARM_LOG_PURE("    [CI] ");
      for (int i = 0; i < 8; ++i) { XARM_LOG_PURE("CI%d=%d, ", i, (cgpio_input_digitals[1] >> i) & 0x0001); }
      XARM_LOG_PURE("\n");
      XARM_LOG_PURE("    [DI] ");
      for (int i = 8; i < 16; ++i) { XARM_LOG_PURE("DI%d=%d, ", i-8, (cgpio_input_digitals[1] >> i) & 0x0001); }
      XARM_LOG_PURE("\n");
      XARM_LOG_PURE("    [AI] AI0=%f, AI1=%f\n", cgpio_input_analogs[0], cgpio_input_analogs[1]);
    XARM_LOG_PURE("CGPIO_OUTPUT: \n");
      XARM_LOG_PURE("    [CO] ");
      for (int i = 0; i < 8; ++i) { XARM_LOG_PURE("CO%d=%d, ", i, (cgpio_output_digitals[1] >> i) & 0x0001); }
      XARM_LOG_PURE("\n");
      XARM_LOG_PURE("    [DO] ");
      for (int i = 8; i < 16; ++i) { XARM_LOG_PURE("DO%d=%d, ", i-8, (cgpio_output_digitals[1] >> i) & 0x0001); }
      XARM_LOG_PURE("\n");
    
      XARM_LOG_PURE("    [AO] AO0=%f, AO1=%f\n", cgpio_output_analogs[0], cgpio_output_analogs[1]);
    XARM_LOG_PURE("CGPIO_CONF: \n");
      XARM_LOG_PURE("    [CI] ");
      for (int i = 0; i < 8; ++i) { XARM_LOG_PURE("CI%d=%d, ", i, cgpio_input_conf[i]); }
      XARM_LOG_PURE("\n");
      XARM_LOG_PURE("    [DI] ");
        for (int i = 8; i < 16; ++i) { XARM_LOG_PURE("DI%d=%d, ", i-8, cgpio_input_conf[i]); }
      XARM_LOG_PURE("\n");
      XARM_LOG_PURE("    [CO] ");
        for (int i = 0; i < 8; ++i) { XARM_LOG_PURE("CO%d=%d, ", i, cgpio_output_conf[i]); }
      XARM_LOG_PURE("\n");
      XARM_LOG_PURE("    [DO] ");
        for (int i = 8; i < 16; ++i) { XARM_LOG_PURE("DO%d=%d, ", i-8, cgpio_output_conf[i]); }
      XARM_LOG_PURE("\n");
  }
  if (total_num >= 481) {
    print_nvect("ft_ext_force = ", ft_ext_force, 6);
...
int XArmReportData::_check_normal_data(unsigned char *rx_data)
{
  ...
  if (rx_data[131+4] < 0 || rx_data[131+4] > 6 || rx_data[132+4] < 0 || rx_data[132+4] > 6) {
    XARM_LOG_ERROR("DataException, collis_sens=%d, teach_sens=%d\n", rx_data[131+4], rx_data[132+4]);
    return 1;
  }
  return 0;
}
