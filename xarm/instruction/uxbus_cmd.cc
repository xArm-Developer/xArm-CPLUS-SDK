...
int UxbusCmd::set_tcp_load(fp32 mass, fp32 load_offset[3]) {
  fp32 txdata[4] = {mass, load_offset[0], load_offset[1], load_offset[2]};
  return set_nfp32(UXBUS_RG::SET_LOAD_PARAM, txdata, 4);
}

int UxbusCmd::set_collis_sens(u8 value) {
  return set_nu8(UXBUS_RG::SET_TEACH_SENS, &value, 1);
}

int UxbusCmd::set_teach_sens(u8 value) {
  return set_nu8(UXBUS_RG::SET_TEACH_SENS, &value, 1);
